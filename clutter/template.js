...
`;
    ...
    for(i = 0; i < myTeam.length; i++) {
        ...
        <div class="littleContent">
            ...
            <h3>E-mail: ${myTeam[i]['email']}</h3>
        `
        
        
        const managerLine = `<h3>Office Number: ${myTeam[i]['office']}</h3>`
        const engineerLine = `<h3>GitHub: ${myTeam[i]['github']}</h3>`
        ...
        newArray.join("")
        var realNewArray = JSON.stringify(newArray);
        allBusinessCards.push(newArray);

    }

    allBusinessCards.join("");
